...
import { Search, Globe, MapPin, Briefcase, Users, X, Loader2, Mail, ArrowRight } from "lucide-react";
...
export function ListicleFinder() {
  ...
  const inputCls =
    "min-h-12 w-full rounded-lg border border-border bg-background px-4 text-[15px] text-foreground outline-none transition-colors placeholder:text-muted focus:border-coral/50 focus-visible:ring-2 focus-visible:ring-coral/30";

  return (
    <div className="mt-10">
      <form
        onSubmit={openModal}
        className="rounded-[var(--radius-xl)] border border-border bg-surface p-5 shadow-[var(--shadow-panel)] sm:p-7"
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Labeled icon={Search} label="Keyword" required>
            <input
              value={f.keyword}
          ...
          <Labeled icon={Globe} label="Your website">
            <input
              value={f.website}
          ...
          <Labeled icon={Users} label="Competitors (optional)">
            <input
              value={f.competitors}
          ...
          <Labeled icon={Briefcase} label="Industry (optional)" full>
            <input
              value={f.industry}
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="font-[family-name:var(--font-mono)] text-[11px] text-muted">
            We find listicles ranking for your keyword and flag where competitors beat you.
          </p>
          <Button
            type="submit"
            size="lg"
...
function Labeled({
  ...
  return (
    <div className={cn(full && "sm:col-span-2")}>
      <label className="mb-1.5 flex items-center gap-1.5 font-[family-name:var(--font-mono)] text-[11px] uppercase tracking-[0.14em] text-muted">
        <Icon size={12} />
        {label}
